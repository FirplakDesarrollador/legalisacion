...
import { ShieldCheck, CheckCircle2, XCircle, FileText, User, Building, Calendar, AlertCircle, Database, UserCheck, X } from 'lucide-react';
  ...
  const handleReasignar = async () => {
    ...
    try {
      ...
      const updateFields: any = {
        observacionesAprobacion: legalizacion.observacionesAprobacion
      ...
      try {
        const link = `${window.location.origin}/formulario-publico/${legalizacion.id}`;
        await fetch('/api/notify', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            correo: newApprover.email,
            titulo: `Aprobación Reasignada - Caja Menor ${legalizacion.codigo}`,
            contenido: `Se te ha reasignado la aprobación de la caja menor ${legalizacion.codigo} de ${legalizacion.usuarioNombre} por valor de ${formatCOP(legalizacion.totalGastos)} COP.${motivoReasignacion.trim() ? ` Motivo: ${motivoReasignacion.trim()}` : ''}`,
            link: link,
          }),
        });
      } catch (notifErr) {
      ...
      setLegalizacion({
        ...legalizacion,
        observacionesAprobacion: updateFields.observacionesAprobacion,
      });
